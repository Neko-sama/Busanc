...
router.get('/borrar/*', function (req, res) {
    ...
    Url = Url.replace('/mantenedores.html/borrar/', ''); 
    //res.send(Url);
...
router.get('/', checkAuth, function (req, res) {
    ...
    conn.connect().then(function () {
        var req = new sql.Request(conn);
        req.query("select * from Usuarios where borrado_suave = 0;").then(function (recordset) {
...
router.post('/crear', function (req, res) {
    ...
    var nombre = req.body.nombre.replace('"', '').replace('"', '');
    var apellido = req.body.apellido.replace('"', '').replace('"', '');
    var correo = req.body.correo.replace('"', '').replace('"', '');
    var pass = req.body.pass.replace('"', '').replace('"', '');
    ...
    res.redirect('/redirect/mantenedores');
