...
router.get('/borrar/*', function (req, res) {
    ...
    Url = Url.replace('/registros.html/borrar/','');
    //res.send(Url);
    var deletequery = "update Indice2 set borrado_suave = 1 where id =" + Url + "; update Solicitud set borrado_suave = 1 where id = " + Url + ";";
    ...
    conn.connect().then(function () {
        ...
        req.query("select * from Solicitud2 where borrado_suave = 0 Order by Id;").then(function (recordset) {
            req.query("select * from Indice2  where borrado_suave = 0 Order by Id;").then(function (query2) {
                ...
                for (var flag = 0; flag < x; flag++) {
                    resultado = JSON.stringify(query2[flag].Creado_por).replace('"', '').replace('"', '').replace('""', '').replace('/\//g', '')
