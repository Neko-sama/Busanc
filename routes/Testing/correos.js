...
router.get('/', checkAuth, function (req, res) {
    ...
    conn.connect().then(function () {
        ...
        req.query("select * from Correos where id = 1;").then(function (recordset) {
            fase1 = JSON.stringify(recordset[0].Fase3).replace('"', '').replace('"', '');;
            fase2 = JSON.stringify(recordset[0].Fase3).replace('"', '').replace('"', '');;
            res.render('correos', {
                title: "Correos",
                fase1: fase1,
                fase2: fase2,
...
router.post('/alter', checkAuth, function (req, res) {
    ...
    var querysalida = "UPDATE Correos SET Fase3= '" + Fase1 + "', Fase4 = '" + Fase2 + "' where id =1 ";
